...
`TEN_INVITE_BONUS_THRESHOLD=10` 表示额外奖励每满 10 个有效邀请发放一次。
...
- `/inventory` 查看 CDK 库存、已使用数量、待补发数量
- `/stats` 查看后台总览
- `/top` 查看当天有效邀请 TOP 5，0 邀请不上榜，管理员不上榜，默认按北京时间统计
- `/user 123456789` 查看某个邀请人的邀请链接、奖励数和最近 20 条被邀请人记录
- `/syncrewards` 同步已达标但未发放的奖励，适合新增奖励规则后执行一次
- `/admin` 查看管理员命令
...
8. 确认 Vercel 环境变量 `ADMIN_IDS=8606512131` 后重新部署
9. 用 `/addcdk` 导入 CDK
